...
class TestCruiseStatusFail < Test::Unit::TestCase
  ...
  def setup
    RakeCommitTask::CruiseStatus.any_instance.expects(:open).with('ccrb.rss').returns(stub(:read => FAIL_RESPONSE))
    @cruise_checker = RakeCommitTask::CruiseStatus.new 'ccrb.rss'
  end
...
class TestCruiseStatusFailOnPointRevision < Test::Unit::TestCase
  ...
  def setup
    RakeCommitTask::CruiseStatus.any_instance.expects(:open).with('ccrb.rss').returns(stub(:read => FAIL_RESPONSE_ON_POINT_REVISION))
    @cruise_checker = RakeCommitTask::CruiseStatus.new 'ccrb.rss'
  end
...
class TestCruiseStatusPass < Test::Unit::TestCase
  ...
  def setup
    RakeCommitTask::CruiseStatus.any_instance.expects(:open).with('ccrb.rss').returns(stub(:read => PASS_RESPONSE))
    @cruise_checker = RakeCommitTask::CruiseStatus.new 'ccrb.rss'
  end
...
class TestCruiseStatusCannotConnect < Test::Unit::TestCase
  ...
  def test_pass_is_false_when_cannot_connect_to_cruise
    RakeCommitTask::CruiseStatus.any_instance.expects(:open).with('bad_url').raises(Exception, 'Cannot connect')
    assert_equal false, RakeCommitTask::CruiseStatus.new('bad_url').pass?
  end
end
